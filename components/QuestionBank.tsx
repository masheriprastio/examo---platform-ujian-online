                    ...
                    {(editingQuestion.options || ['', '', '', '']).map((opt, idx) => (
                      <div key={idx} className="relative group">
                        ...
                        <button 
                          ...
                          className={`absolute right-3 top-3 p-1.5 rounded-lg ${editingQuestion.correctAnswerIndex === idx ? 'bg-green-600 text-white' : 'text-gray-200 hover:text-green-500'}`}
                        >
                          <Check className="w-4 h-4" />
                        </button>
                        <div className="absolute left-4 top-4 font-black text-xs text-gray-300">{String.fromCharCode(65 + idx)}</div>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {(editingQuestion.options || ['', '', '', '']).map((opt, idx) => (
                      <div key={idx} className="relative group">
                        ...
                        <button 
                          ...
                          className={`absolute right-3 top-3 p-1.5 rounded-lg ${editingQuestion.correctAnswerIndices?.includes(idx) ? 'bg-green-600 text-white' : 'text-gray-200 hover:text-green-500'}`}
                        >
                          <Check className="w-4 h-4" />
                        </button>
                        <div className="absolute left-4 top-4 font-black text-xs text-gray-300">{String.fromCharCode(65 + idx)}</div>
